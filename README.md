...
- **工况多变**：不同正极材料（如 NCM 和 NCA）、环境温度（如低温 4℃）和充放电倍率导致电池老化模式差异巨大。
...
## 📈 实验结果
研究在两个数据集（A系列 NCM 电池，B系列 NASA NCA 电池）上验证了模型，涵盖不同温度 (24℃, 4℃) 和倍率 (1C, 2C)。
...
> *图 (a) 展示了 SDTL 在三个不同系列电池上的估计效果；图 (b) 重点展示了在低温 (4℃) 工况下的拟合情况；图 (c) 为误差分布对比。*
